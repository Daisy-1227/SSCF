...
def cast_rays(mesh, rays, max_dist=float('inf')):
    ...
    starts = wp.array(rays[..., :3].contiguous(), dtype=wp.vec3)
    dirs = wp.array(rays[..., 3:].contiguous(), dtype=wp.vec3)
    dists = wp.array(np.full((n,), float('inf'), device=device).float())
...
def closest_point(mesh, points, max_dist=float('inf'), device='cuda'):
    ...
    points_warp = wp.array(points, dtype=wp.vec3)
    ...
    return closest.numpy(), distance.numpy(), triangle_id.numpy(), sign.numpy()


def mesh_distance(mesh, points, max_dist=float('inf'), device='cuda'):
    """nearest-point query on mesh using cuda
    ...
    return sign.numpy() * distance.numpy()