...
from src.human.hand import load_mano,forward_mano
...
class SSCFOptimizer():
    def __init__(self,ckpt_path,optimizer='vL',debug=False):
    ...
    def get_nearest_grid_points(self,points,z,resolution=128,scale=1.5):
        ...
        with torch.no_grad():
            grid_temp = self.O_model.inference(grid[None],z=z)['pts_template'][0]
    ...
    def optimize_v_temp(self,v_init,z,v_temp):        
        ...
        for i in tqdm(range(30)):
            opt.zero_grad()
            v_temp_opt = self.O_model.inference(v_opt[None],z=z)['pts_template'][0]
    ...
    def optimize_v(self,param_init,v_ref,O_sdf=None):
        ...
        def step():
            opt.zero_grad()
            v = forward_mano(self.H_model,**param_opt).verts[0]
            ...
            grads_clip(losses,param_opt,0.002,True)
    ...
    def optimize_vL(self,param_init,v_ref,O,edges,L_ref,O_sdf=None):
        ...
        def step():
            ...
            # grads_clip(losses,param_opt,0.002,True)
            grads_clip(losses,param_opt,0.002,norm=True)
    ...
    def transfer(self,H_src_param,O_src,O_tgt):
        ...
        with torch.no_grad():
            ...
            O_src_feat = self.O_model.inference(O_src_norm[None],z=O_src_z)
            H_src_feat = self.O_model.inference(H_src_norm[None],z=O_src_z)

            # map target object to template field
            O_tgt_z = self.O_model.encoder(O_tgt_norm[None])
            O_tgt_feat = self.O_model.inference(O_tgt_norm[None],z=O_tgt_z)
