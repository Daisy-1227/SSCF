...
def implicit_loss(x,y):
    ...
    embeddings = y['z']
    if 'normals' in  y:
        gt_normals = x['normals']
        pred_normals = y['normals']
    ...
    if 'normals' in y:
        normal_constraint = torch.where(gt_sdf == 0, 1 - F.cosine_similarity(pred_normals, gt_normals, dim=-1)[..., None], torch.zeros_like(pred_normals[..., :1]))
        grad_constraint = torch.abs(pred_normals.norm(dim=-1) - 1)
    ...
    if 'normals' in y:
        ...
        loss_dict['grad_constraint'] = grad_constraint.mean() * 5e1
