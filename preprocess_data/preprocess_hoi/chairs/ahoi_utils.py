...
AHOI_ROOT = 'preprocess_data/preprocess_hoi/chairs/AHOI_Data/AHOI_ROOT'
...
def create_mat_torch(rot, transl, rot_type='matrix', affine=False):
    ...
    if rot_type =='rot_vec':
        ...
        rot = torch.tensor(Rotation.from_rotvec(rot.numpy()).as_matrix())
    elif rot_type != 'matrix':
        rot_type = rot_type[::-1]
        rot = torch.tensor(Rotation.from_euler(rot_type, rot.numpy()).as_matrix())
